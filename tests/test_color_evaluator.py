...
from src.core.color_evaluator import (
    ColorEvaluator,
    InspectionResult,
    ZoneResult,
    ColorEvaluationError
)
...
def good_zones():
    ...
    return [
        Zone(
            ...
            mean_L=50.5, mean_a=20.5, mean_b=10.5,  # 기준값에 가까움
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        ),
        Zone(
            name='B', r_start=0.6, r_end=0.3,
            mean_L=60.2, mean_a=10.3, mean_b=30.1,  # 기준값에 가까움
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        ),
        Zone(
            name='C', r_start=0.3, r_end=0.0,
            mean_L=70.1, mean_a=5.1, mean_b=50.2,  # 기준값에 가까움
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        )
    ]
...
def bad_zones():
    ...
    return [
        Zone(
            name='A', r_start=1.0, r_end=0.6,
            mean_L=50.0, mean_a=20.0, mean_b=10.0,  # OK
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        ),
        Zone(
            name='B', r_start=0.6, r_end=0.3,
            mean_L=70.0, mean_a=30.0, mean_b=10.0,  # 크게 벗어남 (NG)
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        ),
        Zone(
            name='C', r_start=0.3, r_end=0.0,
            mean_L=70.0, mean_a=5.0, mean_b=50.0,  # OK
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        )
    ]
...
def test_evaluate_with_mix_check(sample_sku_config):
    ...
    zones_with_mix = [
        Zone(
            name='A', r_start=1.0, r_end=0.7,
            mean_L=50.0, mean_a=20.0, mean_b=10.0,
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        ),
        Zone(
            name='A-B', r_start=0.7, r_end=0.5,
            mean_L=55.0, mean_a=15.0, mean_b=20.0,  # A와 B의 중간
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='mix'
        ),
        Zone(
            name='B', r_start=0.5, r_end=0.0,
    ...
    config_with_mix['zones']['A-B'] = {
        'L': 55.0, 'a': 15.0, 'b': 20.0, 'threshold': 3.0
    }

    evaluator = ColorEvaluator(sku_config=config_with_mix)

    result = evaluator.evaluate_with_mix_check(
        zones_with_mix,
        'SKU_001',
        check_mix_zones=True
    )
...
def test_evaluate_timestamp():
    ...
    config = {
        'zones': {
            'A': {'L': 50.0, 'a': 20.0, 'b': 10.0, 'threshold': 3.0}
        }
    }

    zone = Zone(
        name='A', r_start=1.0, r_end=0.0,
        mean_L=50.5, mean_a=20.5, mean_b=10.5,
        std_L=1.0, std_a=0.5, std_b=0.5,
        zone_type='pure'
    )
...
def test_evaluate_missing_zone_in_config(sample_sku_config):
    ...
    zones_with_unknown = [
        Zone(
            name='A', r_start=1.0, r_end=0.5,
            mean_L=50.0, mean_a=20.0, mean_b=10.0,
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        ),
        Zone(
            name='UNKNOWN', r_start=0.5, r_end=0.0,
            mean_L=100.0, mean_a=50.0, mean_b=50.0,
            std_L=1.0, std_a=0.5, std_b=0.5,
            zone_type='pure'
        )
    ]
...
def test_evaluate_custom_threshold():
    ...
    zone_marginal = Zone(
        name='A', r_start=1.0, r_end=0.0,
        mean_L=54.0, mean_a=24.0, mean_b=14.0,  # ΔE ~3.5 예상
        std_L=1.0, std_a=0.5, std_b=0.5,
        zone_type='pure'
    )
