...
def test_refine_center_basic(circle_image):
    ...
    assert not np.isclose(refined_cx, initial_cx) or not np.isclose(
        refined_cy, initial_cy
    )  # 정교화가 실제로 이루어졌는지 확인
...
# Test Case 19: ROI가 이미지 경계를 벗어날 때 클램핑 테스트 (Critical Bug Fix)
def test_detect_roi_edge_boundary():
    """이미지 경계 근처에 렌즈가 있을 때 ROI 클램핑 테스트"""
    # 좌상단 모서리에 렌즈가 있는 이미지 (일부만 보임)
    img = np.zeros((300, 300, 3), dtype=np.uint8)
    cv2.circle(img, (50, 50), 60, (255, 255, 255), -1)  # 좌상단 근처
    ...
    assert x >= 0, f"ROI x should be >= 0, got {x}"
    assert y >= 0, f"ROI y should be >= 0, got {y}"

    # ROI가 이미지 경계를 벗어나지 않아야 함
    assert x + w <= img.shape[1], f"ROI right edge {x+w} exceeds image width {img.shape[1]}"
    assert y + h <= img.shape[0], f"ROI bottom edge {y+h} exceeds image height {img.shape[0]}"

    # Width와 height가 양수여야 함
    assert w > 0, f"ROI width should be > 0, got {w}"
    assert h > 0, f"ROI height should be > 0, got {h}"


# Test Case 20: 우하단 모서리 렌즈 ROI 클램핑
def test_detect_roi_bottom_right_corner():
    """우하단 모서리에 렌즈가 있을 때 ROI 클램핑"""
    img = np.zeros((300, 300, 3), dtype=np.uint8)
    cv2.circle(img, (250, 250), 60, (255, 255, 255), -1)  # 우하단 근처
