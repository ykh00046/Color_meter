...
def test_update_sku_success(manager, sample_sku_data):
    ...
    updated = manager.update_sku(
        sample_sku_data["sku_code"],
        {"description": "Updated description", "default_threshold": 4.5}
    )
...
def test_invalid_sku_code_format(manager):
    ...
    with pytest.raises(InvalidSkuDataError):
        manager.create_sku(
            sku_code="INVALID_CODE",  # Should be SKU[0-9]+
...
def test_multi_sku_batch_processing():
    ...
    if len(sku001_images) >= 3:
        ...
        results001 = pipeline001.process_batch(
            [str(p) for p in sku001_images],
            'SKU001'
        )
    ...
    if len(sku002_images) >= 3 and (config_dir / 'SKU002.json').exists():
        ...
        results002 = pipeline002.process_batch(
            [str(p) for p in sku002_images],
            'SKU002'
        )
