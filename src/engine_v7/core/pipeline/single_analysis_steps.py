"""
Single Analysis Helper Functions

Extracted from single_analyzer.py.
Contains all internal analysis step functions called by analyze_single_sample().

- Color histogram analysis
- Radial profile analysis
- Ink segmentation analysis
- Pattern feature analysis
- Zone analysis
- Quality scoring
- Warning generation
- Operator decision
- Engineer KPI extraction
"""
...
def _analyze_pattern_features(test_bgr: np.ndarray, geom: LensGeometry, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "center_blobs": {
            ...
            "top_3_blobs": representative_blobs,
            "_debug": {
...
def _calculate_quality_score(results: Dict[str, Any]) -> float:
    ...
    Weights (C. None score renormalization):
    - Gate: 30% (renormalization: None scores excluded)
    - Color consistency: 20%
    - Pattern quality: 20% (angular uniformity - how uniform around 360 deg)
    - Zone uniformity: 15%
    ...
    # C. Gate score - None renormalization average (decision_builder method)
    def _weighted_gate_score(gate_scores: dict) -> float:
        """Exclude None scores with weight renormalization"""
        items = []
    ...
    # C. Renormalized gate score (0-1 scale)
    gate_normalized = _weighted_gate_score(gate_raw_scores)
    # passed=False -> 0 score (quality gate failure)
