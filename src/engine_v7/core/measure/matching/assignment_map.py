...
def _infer_center_space(centers: np.ndarray, l_weight: float) -> str:
    """
    Heuristic inference for centers layout.
    Returns:
        'LAB' meaning [L, a, b] or 'ABL' meaning [a, b, L*l_weight]
    """
    if centers.size == 0:
        return "LAB"

    a_like = np.all((centers[:, 0] >= -128) & (centers[:, 0] <= 127))
    b_like = np.all((centers[:, 1] >= -128) & (centers[:, 1] <= 127))
    l_like = np.all((centers[:, 0] >= 0) & (centers[:, 0] <= 100))
    third_small = np.all(centers[:, 2] <= max(1.0, 100.0 * float(l_weight) * 1.2))

    if a_like and b_like and third_small:
        return "ABL"
    if l_like:
        return "LAB"
    return "LAB"
...
def _normalize_centers(centers: np.ndarray, l_weight: float, center_space: str) -> np.ndarray:
    """
    Convert centers to feature space [a, b, L*l_weight].
    """
    space = str(center_space).upper()
    if space == "AUTO":
        space = _infer_center_space(centers, l_weight)
    if space == "LAB":
        return np.stack(
            [
                centers[:, 1],
                centers[:, 2],
                centers[:, 0] * float(l_weight),
            ],
            axis=1,
        )
    if space == "ABL":
        return centers.astype(np.float32, copy=False)
    raise ValueError(f"center_space must be LAB, ABL, or AUTO, got {center_space}")
...
def build_assignment_map(
    ...
    *,
    ...
    centers_feat = _normalize_centers(centers, l_weight, center_space)
    k = centers_feat.shape[0]
    ...
    distances = np.sum((roi_pixels[:, None, :] - centers_feat[None, :, :]) ** 2, axis=2)
