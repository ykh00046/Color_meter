...
from ...utils import CIELabArray, lab_cv8_to_cie, to_cie_lab

# ==============================================================================
# Legacy functions (deprecated - use utils.to_cie_lab instead)
# ==============================================================================
...
def lab_opencv_to_cie(lab: np.ndarray) -> np.ndarray:
    ...
    .. deprecated::
        Use `lab_cv8_to_cie` from utils instead.
    """
    warnings.warn(
        "lab_opencv_to_cie() is deprecated. Use utils.lab_cv8_to_cie() instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    return lab_cv8_to_cie(lab)


def ensure_cie_lab(lab: np.ndarray) -> CIELabArray:
    """
    Heuristic: if a/b look like 0~255 scale, treat as OpenCV and convert.

    .. deprecated::
        Use `to_cie_lab(lab, source="auto")` from utils instead.
    """
    # Note: No warning here as this function is still widely used.
    # We delegate to to_cie_lab which has the same auto-detection logic.
    return to_cie_lab(lab, validate=False)
...
def _to_cie_vec(lab: Any) -> np.ndarray:
    ...
    Uses to_cie_lab for auto-conversion.
    """
    x = np.asarray(lab, dtype=np.float32).reshape(
        3,
    )
    try:
        y = to_cie_lab(x.reshape(1, 3), validate=False).reshape(
            3,
        )
