...
from ..metrics.ink_metrics import build_cluster_stats
...
def _segment_image(
    ...
    for c in clusters:
        if "mean_lab" in c:
            c["mean_lab"] = (
                to_cie_lab(np.array(c["mean_lab"], dtype=np.float32).reshape(1, 3), validate=False).reshape(3).tolist()
            )
