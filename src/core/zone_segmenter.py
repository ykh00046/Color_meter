...
class SegmenterConfig:
    ...
    detection_method: str = "variable_width"  # gradient, delta_e, hybrid, variable_width (PHASE7 Priority 7)
...
class ZoneSegmenter:
    ...
    def diagnostics(
        ...
        r_max = float(profile.r_normalized[-1]) if len(profile.r_normalized) else 1.0
        r_min = float(profile.r_normalized[0]) if len(profile.r_normalized) else 0.0
        boundaries_full = [r_max] + boundaries + [r_min]
        boundaries_full = self._merge_close_boundaries(boundaries_full, self.config.min_zone_width)

        return {
            "detection_method": self.config.detection_method,
            "uniform_split_priority": self.config.uniform_split_priority,
            "min_zone_width": self.config.min_zone_width,
            "r_inner": float(r_inner),
            "r_outer": float(r_outer),
            "expected_zones": hint_zones,
            "profile_points": int(len(profile.r_normalized)),
            "gradient_peaks": [float(v) for v in grad_pts],
            "delta_e_peaks": [float(v) for v in de_pts],
            "boundaries": [float(v) for v in boundaries_full],
        }
