...
class RadialProfiler:
    ...
    def extract_profile(self, image: np.ndarray, lens: LensDetection) -> RadialProfile:
        ...
        polar_image = cv2.warpPolar(
            image,
            (r_samples, self.config.theta_samples),
            (cx, cy),
            radius,
        ...
        # Cast to ndarray for mypy (we know these are arrays from the input types)
        L_std_arr = np.asarray(L_std)
