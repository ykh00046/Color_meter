...
def profile_radial_profiling_detailed():
    ...
    polar_image = cv2.warpPolar(
        processed, (r_samples, profiler.config.theta_samples),
        (cx, cy), radius,
    ...
    for theta in [90, 180, 360, 720]:
        ...
        print(f"theta={theta:>4}: {elapsed:>8.2f}ms  (speedup: {t1+t2+t3:>6.2f}ms / {elapsed:>6.2f}ms = {(t1+t2+t3)/elapsed:.2f}x)")
