...
class PerformanceProfiler:
    ...
    def profile_single_image(self, image_path: Path, sku_config: dict) -> Tuple[List[ProfilingResult], float]:
        """
        단일 이미지 처리 프로파일링.
...
def print_results(results: List[ProfilingResult], total_time: float):
    ...
    for r in results:
        ...
        print(
            f"{r.step_name:<30} {r.elapsed_ms:>10.2f}   {percentage:>6.1f}%  "
            f"{r.memory_mb:>10.2f}   {r.cpu_percent:>6.1f}%"
        )
...
def main():
    ...
    print(
        f"BOTTLENECK: {slowest.step_name} ({slowest.elapsed_ms:.2f}ms, "
