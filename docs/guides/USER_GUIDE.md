...
### 3.3 SKU 목록 조회
현재 등록된 SKU들의 목록을 확인하거나 세부 정보를 보려면 **웹 UI**를 사용하세요.

> **참고**: CLI 도구 (`src/main.py`)는 2025-01-12에 제거되었습니다. 모든 기능은 웹 UI를 통해 사용할 수 있습니다.

웹 서버를 실행한 후 브라우저에서:
1. `http://localhost:8888/` 접속
2. 상단 메뉴에서 "History" 또는 "Stats" 페이지로 이동
3. 등록된 SKU 목록 및 검사 내역 확인

또는 `config/sku_db/` 폴더의 JSON 파일을 직접 확인할 수 있습니다.

## 4. 검사항목 실행

> **중요**: CLI 도구 (`src/main.py`)는 2025-01-12에 제거되었습니다. 모든 검사 기능은 **웹 UI**를 통해 사용할 수 있습니다.

이 시스템은 **웹 인터페이스(UI)**를 통해 검사를 실행합니다. 웹 UI는 단일 이미지 검사와 배치 검사를 모두 지원합니다.

### 4.1 웹 서버 시작
터미널에서 다음 명령으로 웹 서버를 시작합니다:
```bash
# 방법 1: 배치 스크립트 사용 (Windows)
scripts\run_web_ui_noreload.bat
...
### 4.2 웹 UI를 통한 검사
1. **브라우저 접속**: `http://localhost:8888` 접속
2. **단일 이미지 검사**:
   - 홈페이지에서 이미지 업로드
   - SKU 코드 선택
   - "Analyze" 버튼 클릭
...
검사 결과는 `results/` 폴더에 자동 저장되며, 웹 UI에서 실시간으로 확인할 수 있습니다.
...
### 5.2 결과 파일 확인
웹 UI 검사 후 생성된 결과 이미지/파일을 직접 열어볼 수 있습니다.
...
3. **프로파일 집계 방식:** `params.sample_percentile`(0~100)을 설정하면 프로파일 평균 대신 분위수 집계가 적용됩니다. 노이즈가 심할 때 중앙값(50)을 시도해볼 수 있습니다.
